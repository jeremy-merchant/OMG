...
9. Observe Git through OMG without reset, clean, checkout, merge, rebase, commit, push, worktree deletion, or other mutation unless separately authorized. Use payload-free ` + "`omg git current|latest|history --project <root> --json`" + ` and payload-free ` + "`omg git diff --project <root> --json`" + `. Do not invent a ` + "`session_id`" + ` filter or inspect unrelated repositories.
