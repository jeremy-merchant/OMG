...
var completionCommandDescriptions = map[string]string{
	...
	"mode":        "Classify boundary-ledger scope and return the controller/worker command policy.",
	...
	"candidate":   "Inspect or safely close one FULL lifecycle candidate without executing Canary or mutating Git.",
...
var completionValueDescriptions = map[string]map[string]string{
	...
	"worker":      {"setup": "Create or validate worker execution state in one request.", "bootstrap": "Ensure worker identity and task ownership, then return its scoped board and inbox."},
	...
	"candidate":   {"close": "Inspect missing gates or atomically close an evidence-complete FULL lifecycle."},
	"reserve":     {"add": "Add one advisory path reservation.", "batch-add": "Atomically add multiple reservations for one owner.", "list": "List reservation records.", "active": "List active reservations.", "history": "Read reservation history.", "renew": "Renew a live reservation.", "release": "Release a reservation with a reason.", "override": "Record a human override without hiding conflict."},
...
var completionByCommand = map[string][]string{
	...
	"candidate":   {"close"},
	"reserve":     {"add", "batch-add", "list", "active", "history", "renew", "release", "override"},
