...
1. **Git project mode (default when inside a non-bare Git worktree):** run
   `git rev-parse --path-format=absolute --git-common-dir` with the project worktree as the Git context. The normalized common-directory identity is mapped to `<platform-user-state-root>/omg/git/<stable-common-dir-id>/state.db`.
   - The common directory, rather than `.git` as seen from a linked worktree, is the identity authority. Every linked worktree for one repository therefore resolves the same database while mutable state remains outside Git metadata.
   ...
   - The store directory and SQLite sidecars (`state.db-wal`, `state.db-shm`, rollback journals, backups, lock files, PID files) remain private and untracked. `.omg/` contains only safe, intentionally tracked configuration.
2. **Non-Git project mode:** store state below the platform user-private state root under `omg/projects/<stable-project-id>/state.db`. On macOS that root is the per-user directory returned by `getconf DARWIN_USER_DIR`; other supported systems use their current user-configuration root until a stronger native state root is defined. `stable-project-id` MUST be derived from a normalized absolute root and persisted in the database; it is not a security secret. This avoids placing a database in source or intentionally shared home directories by default.
3. **Explicit workspace mode:** `omg init --workspace <absolute-directory>` creates or joins one named local workspace record. Its database is below the same platform user-private state root at `omg/workspaces/<workspace-id>/state.db`; enrolled project roots are rows in that database. A project belongs to at most one resolved workspace for a command invocation. Workspace mode is selected explicitly by flag or safe project configuration, never by discovering a nearby directory.
...
- Opening/discovery MUST inspect schema metadata and report unapplied migrations as `pending`. `omg preflight` and `omg worker bootstrap` MAY apply only a non-fresh incremental plan for which every compiled migration is explicitly classified `auto-safe`; they MUST first create and verify the exact plan-bound backup, record `automatic_safe_policy` authorization evidence, apply atomically, and pass integrity verification. `omg version`, `omg doctor`, and other status/query commands remain read-only. Mixed, unknown, risky, newer, checksum-mismatched, or partially applied plans fail closed and remain pending.
- `omg migration plan` is the non-mutating discovery surface and reports the ordered pending versions/checksums, expected backup location, stable plan identifier, and automatic-eligibility decision. Any plan not eligible for the narrowly defined automatic-safe policy requires `omg migration apply`, which MUST reject unless the operator supplies an explicit human approval PIN bound to that exact plan identifier, project/fixture, from/to schema versions, migration checksums, verified backup checksum/destination, command, and UTC timestamp; a message, handoff, `SAFE_TO_REMOVE`, dry run, or prior approval is never sufficient.
...
- `omg backup create [--plan-file FILE]`, `omg backup restore --payload-file FILE`, and `omg doctor` use the same adapter. A restore that mutates state requires a separate explicit human-approved apply operation; this ADR authorizes no automatic restore.
...
8. Pending-migration fixtures prove both paths: an eligible incremental all-`auto-safe` plan is backed up, atomically applied, integrity-checked, and recorded with machine-policy evidence by `preflight`; every other plan remains pending until `migration plan` plus separately approved `migration apply`. Fixtures retain the exact plan and secret-free authorization binding.
