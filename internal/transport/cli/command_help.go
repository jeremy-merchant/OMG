...
var helpCommands = []helpCommand{
	...
	{Name: "mode", Group: "START + VERIFY", Summary: "Classify work into a proportional, read-only lifecycle contract.", Usage: []string{"omg mode <observe|work-lite|full> [--json]", "omg mode classify --payload <json> [--json]"}, Subcommands: []helpSubcommand{{Name: "observe", Summary: "Return the no-ledger contract for read-only work."}, {Name: "work-lite", Summary: "Return the single-owner change contract without a mandatory handoff."}, {Name: "full", Summary: "Return the complete coordination, handoff, review, and release contract."}, {Name: "classify", Summary: "Classify stable risk signals without reading or writing project state.", Example: "omg mode classify --payload '{\"mutates_files\":true,\"changes_user_visible_behavior\":false}' --json", PayloadNote: "Accepted signals include mutates_files, creates_branch, creates_worktree, uses_multiple_agents, touches_production, touches_auth_or_payment, changes_user_visible_behavior, external_side_effects, release_or_canary, requires_handoff, expected_duration_minutes, and override."}}},
