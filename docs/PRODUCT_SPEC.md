...
>
> **Working identity and publication status:** OMG (**Oh My Group**), CLI `omg`, tracked configuration directory `.omg/`, environment prefix `OMG_`, and **OMG Coordination Protocol (OMGCP)** are private working identifiers. **Publication status: NOT PUBLISHED.** The working name is not cleared for public distribution. The complete human publication gate is in [NAME_CLEARANCE.md](brand/NAME_CLEARANCE.md); no public repository, package, domain, release, or external distribution is authorized until that gate is satisfied. A later identity change invalidates the local RC pin and requires both proof projects and their parity evidence to be rerun.
...
- [ ] Publication status remains `NOT PUBLISHED` without separate approval.
