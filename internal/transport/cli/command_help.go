...
var helpWorkflows = []helpWorkflow{
	{Marker: "01", Name: "Choose scope", Command: "omg mode classify", Summary: "Use OBSERVE, WORK-LITE, or FULL according to actual coordination risk."},
	{Marker: "02", Name: "Start work", Command: "omg preflight → omg worker bootstrap → omg board me", Summary: "Initialize coordination only when the selected mode requires it."},
	{Marker: "03", Name: "Share state", Command: "omg progress add → omg message send → omg handoff create", Summary: "Publish and transfer state only when another owner or release controller needs it."},
...
var helpCommands = []helpCommand{
	...
	{Name: "preflight", Group: "START + VERIFY", Summary: "Return startup health, mutation_allowed, and exact blocking reasons; keep historical debt in housekeeping.", Usage: []string{"omg preflight [selection] [--session ID] [--verbose] [--json]"}, Options: [][2]string{{"--session <id>", "Select the current session identity"}, {"--verbose", "Include sessions, tasks, inbox, reservations, Git, and actions"}}, Examples: []string{"omg preflight --project /absolute/project", "omg preflight --project /absolute/project --verbose --json"}},
	{Name: "status", Group: "START + VERIFY", Summary: "Show live ownership signals separately from historical housekeeping and workflow debt.", Usage: []string{"omg status [selection] [--json]"}, Examples: []string{"omg status --project /absolute/project --json"}},
	{Name: "stale", Group: "START + VERIFY", Summary: "Classify open sessions by liveness, observability, and unfinished closure risk.", Usage: []string{"omg stale [selection] [--json]"}, Examples: []string{"omg stale --project /absolute/project --json"}},
	...
	{Name: "migration", Group: "START + VERIFY", Summary: "Inspect automatic eligibility or explicitly apply a checksummed schema migration.", Usage: []string{"omg migration <plan|apply> [selection] [options]"}, Subcommands: []helpSubcommand{{Name: "plan", Summary: "Create a read-only plan including automatic eligibility; optionally save it privately.", Example: "omg migration plan --project /project --output plan.json --json"}, {Name: "apply", Summary: "Apply a non-eligible exact plan after validating matching human approval and backup.", Example: "omg migration apply --project /project --plan-file plan.json --approval-file approval.json --json", Mutation: true}}, Options: [][2]string{{"--output <path>", "Save a complete private plan"}, {"--plan-file <path>", "Read the exact private plan"}, {"--approval-file <path>", "Read the matching private approval"}}},
	...
	{Name: "worker", Group: "START + VERIFY", Summary: "Atomically establish a worker execution unit, or bootstrap an already registered worker into its inbox and scoped board.", Usage: []string{"omg worker <setup|bootstrap> [selection] [payload or identity options]"}, Subcommands: []helpSubcommand{{Name: "setup", Summary: "Create or validate session, task, claim, run, and initial reservations in one transaction.", Example: "omg worker setup --project /project --idempotency-key worker-setup-1 --payload-file worker-setup.json --json", Mutation: true, PayloadNote: "Requires an existing human and live controller session plus explicit worker session, task, and run IDs. Supply 0-128 initial reservations. Any conflict rolls back the complete setup."}, {Name: "bootstrap", Summary: "Ensure pre-registered worker identity and task ownership, then return only the worker-scoped next action.", Example: "OMG_PROJECT=/project OMG_SESSION_ID=worker-1 OMG_TASK_ID=TASK_ID OMG_CONTROLLER_SESSION_ID=controller-1 OMG_HUMAN_ID=HUMAN_ID omg worker bootstrap --idempotency-key bootstrap-worker-1 --output /tmp/worker-1.env --json", Mutation: true, PayloadNote: "Reads the five OMG_* identity variables when matching flags are omitted. The optional output is a new owner-only shell environment file."}}},
	{Name: "mode", Group: "START + VERIFY", Summary: "Classify boundary-ledger scope and return the controller/worker command policy.", Usage: []string{"omg mode <observe|work-lite|full> [--json]", "omg mode classify --payload <json> [--json]"}, Subcommands: []helpSubcommand{{Name: "observe", Summary: "Return the no-ledger contract for read-only diagnosis."}, {Name: "work-lite", Summary: "Return the one-start/one-finish contract for one branch and one worker."}, {Name: "full", Summary: "Return the complete integration, review, Canary, deploy, and release contract."}, {Name: "classify", Summary: "Classify stable risk signals without reading or writing project state.", Example: "omg mode classify --payload '{\"mutates_files\":true,\"changes_user_visible_behavior\":false}' --json", PayloadNote: "Accepted signals include mutates_files, creates_branch, creates_worktree, uses_multiple_agents, touches_production, touches_database, touches_auth_or_payment, changes_user_visible_behavior, external_side_effects, release_or_canary, requires_handoff, expected_duration_minutes, and override. Stateful contracts require controller-provided complete commands and prohibit worker command discovery."}}},
	...
	{Name: "task", Group: "COORDINATE WORK", Summary: "Create, inspect, claim, and transition tasks and their execution runs atomically.", Usage: []string{"omg task <subcommand> [selection] [payload options]"}, Subcommands: coordinationSubs([]helpSubcommand{{Name: "create", Summary: "Create a task with an atomic display number.", Example: "omg task create --project /project --idempotency-key task-1 --payload-file task.json --json", Mutation: true}, {Name: "get", Summary: "Read one task and its canonical state.", Example: "omg task get --project /project --payload '{\"task_id\":\"TASK_ID\"}' --json"}, {Name: "claim", Summary: "Claim one ready task for exactly one session.", Example: "omg task claim --project /project --idempotency-key claim-1 --payload '{\"task_id\":\"TASK_ID\",\"session_id\":\"SESSION_ID\"}' --json", Mutation: true}, {Name: "transition", Summary: "Apply a validated task state transition and dependency reconciliation.", Example: "omg task transition --project /project --idempotency-key transition-1 --payload-file transition.json --json", Mutation: true}, {Name: "run-create", Summary: "Create an execution run for a task and session.", Example: "omg task run-create --project /project --idempotency-key run-1 --payload-file run.json --json", Mutation: true}, {Name: "run-transition", Summary: "Apply a validated run state transition.", Example: "omg task run-transition --project /project --idempotency-key run-transition-1 --payload-file run-transition.json --json", Mutation: true}, {Name: "finish-lite", Summary: "Atomically complete one WORK-LITE run/task, release its reservations, and archive its session.", Example: "omg task finish-lite --project /project --idempotency-key finish-lite-1 --payload-file finish-lite.json --json", Mutation: true, PayloadNote: "The actor must be the owning session and every run owned by that session must become terminal."}}), Examples: []string{"omg task create --project /project --idempotency-key task-1 --payload-file task.json --json", "omg task finish-lite --project /project --idempotency-key finish-lite-1 --payload-file finish-lite.json --json"}},
	...
	{Name: "candidate", Group: "INSPECT + INTEGRATE", Summary: "Inspect or safely close one FULL lifecycle candidate without executing Canary or mutating Git.", Usage: []string{"omg candidate close [selection] [payload options]"}, Subcommands: coordinationSubs([]helpSubcommand{{Name: "close", Summary: "Report missing gates, or atomically verify the run/task, release reservations, and archive the source session after SOURCE_CLEANED.", Example: "omg candidate close --project /project --idempotency-key candidate-close-1 --payload '{\"handoff_id\":\"HANDOFF_ID\",\"actor_session_id\":\"CONTROLLER_SESSION_ID\",\"archive_event_id\":\"ARCHIVE_EVENT_ID\",\"evidence\":\"exact real Canary passed and source cleanup verified\"}' --json", Mutation: true, PayloadNote: "Before all gates are present, returns closed:false with missing evidence and a copyable next action without consuming the idempotency key. Never executes Canary or deletes Git assets."}})},
	{Name: "reserve", Group: "COORDINATE WORK", Summary: "Declare path intent, inspect conflicts, renew leases, and record explicit releases or overrides.", Usage: []string{"omg reserve <subcommand> [selection] [payload options]"}, Subcommands: coordinationSubs([]helpSubcommand{{Name: "add", Summary: "Add an advisory path reservation.", Example: "omg reserve add --project /project --idempotency-key reserve-1 --payload '{\"id\":\"reservation-1\",\"pattern_kind\":\"exact\",\"pattern\":\"TODO.md\",\"case_sensitivity\":\"sensitive\",\"mode\":\"exclusive\",\"human_id\":\"HUMAN_ID\",\"session_id\":\"SESSION_ID\",\"task_id\":\"TASK_ID\",\"run_id\":\"RUN_ID\",\"intent\":\"edit TODO\",\"ttl_seconds\":3600}' --json", Mutation: true, PayloadNote: "Requires human_id, session_id, task_id, and run_id. Create the run first with `omg task run-create --help`."}, {Name: "batch-add", Summary: "Atomically add multiple reservations for one owner in one request.", Example: "omg reserve batch-add --project /project --idempotency-key reserve-batch-1 --payload-file reservations.json --json", Mutation: true, PayloadNote: "Supply one owner and 1-128 items. All items are validated and conflict-checked before any reservation is persisted."}, {Name: "list", Summary: "List reservation records."}, {Name: "active", Summary: "List active reservation records."}, {Name: "history", Summary: "Read one reservation history."}, {Name: "renew", Summary: "Renew a live reservation from a checkpoint.", Mutation: true}, {Name: "release", Summary: "Release a reservation with a reason.", Mutation: true}, {Name: "override", Summary: "Record a human override without hiding the conflict.", Mutation: true}})},

	{Name: "board", Group: "INSPECT + INTEGRATE", Summary: "Render operational, hygiene, history, or canonical coordination views.", Usage: []string{"omg board <actionable|backlog|hygiene|history|summary|me|tree|task|all|git> [selection] [selector] [--format FORMAT | --json]"}, Subcommands: []helpSubcommand{{Name: "actionable", Summary: "Show only work that can require an operator decision now."}, {Name: "backlog", Summary: "Show compact state counts and the largest workflow bottlenecks."}, {Name: "hygiene", Summary: "Classify stale and finished-but-unclosed sessions."}, {Name: "history", Summary: "Show the complete append-only snapshot for audit."}, {Name: "summary", Summary: "Count states and expose the largest workflow bottleneck."}, {Name: "me", Summary: "Show one selected session and adjacent work."}, {Name: "tree", Summary: "Show session and task lineage."}, {Name: "task", Summary: "Show one selected task and adjacent coordination facts."}, {Name: "all", Summary: "Show the complete safe operator snapshot."}, {Name: "git", Summary: "Focus the snapshot on Git observations and risks."}}, Options: [][2]string{{"--session <id>", "Required by board me"}, {"--task <id>", "Required by board task"}, {"--format <format>", "tty, markdown, html, or json"}}, Examples: []string{"omg board actionable --project /absolute/project --json", "omg board task --project /project --task TASK_ID --format tty"}},
	{Name: "git", Group: "INSPECT + INTEGRATE", Summary: "Overlay coordination facts on authoritative Git state for only the selected repository and its linked worktrees.", Usage: []string{"omg git <subcommand> [selection] [payload options]"}, Subcommands: coordinationSubs([]helpSubcommand{{Name: "inventory", Summary: "Explicitly record point-in-time Git evidence for an audit or handoff.", Mutation: true}, {Name: "current", Summary: "Inspect live Git without adding OMG records.", Example: "omg git current --project /project --json", PayloadNote: "No payload is required. An optional session_id compatibility hint is accepted but does not change project scope."}, {Name: "latest", Summary: "Read the latest explicitly recorded Git evidence.", Example: "omg git latest --project /project --json", PayloadNote: "No payload is required. An optional session_id compatibility hint is accepted but does not change project scope."}, {Name: "history", Summary: "Read recorded audit evidence, not Git commit history.", Example: "omg git history --project /project --json", PayloadNote: "No payload is required. An optional session_id compatibility hint is accepted but does not change project scope."}, {Name: "diff", Summary: "Compare recorded evidence; use Git itself for code diffs.", Example: "omg git diff --project /project --json", PayloadNote: "No payload compares the latest observations in recorded evidence. Supply before and after observation IDs only to select another pair."}, {Name: "cleanup-plan", Summary: "Inspect live Git and produce a non-destructive advisory cleanup plan.", Example: "omg git cleanup-plan --project /project --json", PayloadNote: "No payload plans all live assets; fingerprint optionally selects one asset. No observation is persisted."}, {Name: "reconcile", Summary: "Prove recorded source SHA/tree inclusion against live Git.", Example: "omg git reconcile --project /project --integration-branch main --json"}, {Name: "adopt", Summary: "Change coordination ownership metadata only; never mutate Git.", Mutation: true}})},
	...
	{Name: "canary", Group: "INSPECT + INTEGRATE", Summary: "Pin strict ledger-backed or local Git-evidence exact-SHA canary receipts; OMG records evidence but does not execute the command.", Usage: []string{"omg canary <start|finish> [selection] [options]"}, Subcommands: []helpSubcommand{{Name: "start", Summary: "Use strict ledger evidence by default, or opt into local rolling Git evidence with an explicit candidate SHA.", Example: "omg canary start --project /project --handoff HANDOFF --session SESSION --integration-ref refs/heads/rolling --canary-mode local_integration --candidate-sha CANDIDATE_SHA --verification-command 'go test ./...' --execution-kind real --environment-fingerprint ENV_HASH --idempotency-key KEY --json", Mutation: true, PayloadNote: "Default and release_or_production require the INTEGRATED ledger event. local_integration requires the exact candidate SHA to be reachable from the rolling ref and the rolling worktree to be clean; a missing ledger event is recorded as pending reconciliation instead of blocking."}, {Name: "finish", Summary: "Record counts and invalidate automatically if the integration ref moved.", Example: "omg canary finish --project /project --canary CANARY_ID --session SESSION --exit-code 0 --passed 1 --failed 0 --skipped 0 --evidence-path /path/to/log --idempotency-key KEY --json", Mutation: true}}},
...
func globalCommandSummary(name string) string {
	summaries := map[string]string{
		...
		"worker":      "Set up a worker execution unit or bootstrap its scoped task and inbox.",
