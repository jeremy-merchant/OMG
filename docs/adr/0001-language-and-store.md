...
- Opening/discovery MUST inspect schema metadata and report unapplied migrations as `pending`. `omg preflight` and `omg worker bootstrap` MAY apply every non-empty exact plan compiled into the installed binary; they MUST first create and verify the exact plan-bound backup, record `automatic_safe_policy` machine authorization evidence, apply atomically, and pass integrity verification. `omg version`, `omg doctor`, and other status/query commands remain read-only. Unknown, newer, checksum-mismatched, stale, partially applied, backup-failed, or integrity-failed plans fail closed and remain pending.
- `omg migration plan` is the non-mutating discovery surface and reports the ordered pending versions/checksums, expected backup location, stable plan identifier, and automatic-eligibility decision. Every non-empty exact compiled plan is eligible for the automatic backup policy. The legacy manual `omg migration apply` compatibility path still rejects unless the operator supplies its exact bounded approval file; a message, handoff, `SAFE_TO_REMOVE`, dry run, or prior approval is never sufficient for that manual command.
...
8. Pending-migration fixtures prove fresh, incremental, and mixed compiled plans are backed up, atomically applied, integrity-checked, and recorded with machine-policy evidence by `preflight`; unknown, stale, checksum-divergent, failed, or corrupt plans remain pending. Separate fixtures retain the legacy manual apply validation contract.
